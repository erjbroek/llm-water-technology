...
class RAGChatbot:
    ...
    def get_relevant_context(self, query, language):
        # Retrieve relevant document chunks for the query from both languages.
        # Merges results and selects the absolute top 5 based on vector distance.

        # --- Fix: Default to English if language is None ---
        if not language:
            language = "english"
        
        CANDIDATE_LIMIT = 5  # Get top 5 from EACH language to compete
        FINAL_LIMIT = 5      # Return top 5 OVERALL
        
        try:
            # Prepare queries with labels
            queries_with_labels = []
            
            # 1. Setup Original Query
            queries_with_labels.append({"text": query, "label": f"Original ({language})"})

            # 2. Setup Translated Query
            if language.lower() == "dutch": 
                try:
                    translator = GoogleTranslator(source="nl", target="en")
                    english_query = translator.translate(query)
                    queries_with_labels.append({"text": english_query, "label": "Translated (English)"})
                    print(f"DEBUG: Translated query (NL->EN): {english_query}")
                except Exception as e:
                ...
                try:
                    ...
                    queries_with_labels.append({"text": dutch_query, "label": "Translated (Dutch)"})
                    print(f"DEBUG: Translated query (EN->NL): {dutch_query}")
                except Exception as e:
                    print(f"Translation failed: {e}")

            # 3. Execute Searches and Pool Results
            pooled_chunks = {}

            for q_obj in queries_with_labels:
                q_text = q_obj["text"]
                q_label = q_obj["label"]
                ...
                results = self.vector_store.search(q_text, top_k=CANDIDATE_LIMIT)
                ...
                dists = results['distances'][0] if 'distances' in results else [1.0] * len(ids)

                for chunk_id, doc, meta, dist in zip(ids, docs, metas, dists):
                    if chunk_id not in pooled_chunks:
                        pooled_chunks[chunk_id] = {
                            'text': doc, 
                            'score': dist,
                        ...
                        if dist < pooled_chunks[chunk_id]['score']:
                            pooled_chunks[chunk_id]['score'] = dist
                            pooled_chunks[chunk_id]['origin'] = q_label

            # 4. Sort by score (Distance: Lower is better)
            sorted_candidates = sorted(
                pooled_chunks.values(), 
                key=lambda x: x['score']
            )

            # 5. Select Final Top Chunks
            top_chunks = sorted_candidates[:FINAL_LIMIT]
            
            # --- DEBUG PRINTING (Visible in Terminal) ---
            print(f"\n{'='*10} RETRIEVAL WINNERS {'='*10}")
            print(f"User Query: {query}")
            for i, chunk in enumerate(top_chunks):
                print(f"Rank {i+1}: [{chunk['origin']}] Score: {chunk['score']:.4f} | File: {chunk['filename']}")
            print(f"{'='*38}\n")
            # ---------------------------------------------
            
            final_docs = [item['text'] for item in top_chunks]
            return "\n\n".join(final_docs)
...
class RAGEvaluator:
    def __init__(self, chatbot):
        ...
        # JSON is in the SAME folder as backend.py
        base_dir = os.path.dirname(__file__)
        json_path = os.path.join(base_dir, "questions_answers.json")

        with open(json_path, "r", encoding="utf-8") as f:
            evaluation_questions_and_answers = json.load(f)
        ...
        print(f"Loaded JSON successfully: {len(self.prepared_qa_nl)} NL, {len(self.prepared_qa_en)} EN QA pairs.")
