...
class VectorStore:
    ...
    def document_is_unchanged(self, document_id: str, fingerprint: Dict[str, Any]) -> bool:
        ...
        try:
            return (
                int(old_meta.get("source_mtime")) == int(fingerprint["source_mtime"])
                and old_meta.get("source_size") == fingerprint["source_size"]
            )
...
def ingest_file(
    ...
    # Makes sure files are only chunked if a new or updated document is found
    if not force and vector_store.document_is_unchanged(document_id, fingerprint):
        return False, f"Unchanged: {document_id}"
...
def ingest_directory(
    ...
    logger.info(f"There are {len(all_files)} files found")
